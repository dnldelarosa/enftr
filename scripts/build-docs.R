...
output <- if (length(args)) args[[1]] else file.path(workspace, "artifacts/enft-release/sites/r")
...
.libPaths(c(file.path(workspace, "artifacts/dmisc-release/package/zip-installed"),
  file.path(workspace, "artifacts/dmisc-release/library"),
  file.path(workspace, "artifacts/labeler/bilingual/package/labeler.Rcheck"), .libPaths()))
...
library(enftr)
if (packageVersion("enftr") != as.character(read.dcf(file.path(pkg, "DESCRIPTION"))[1, "Version"])) stop("Wrong source package version")
Sys.setenv(R_LIBS = paste(.libPaths(), collapse = .Platform$path.sep))
...
for (entry in c("DESCRIPTION", "LICENSE", "NAMESPACE", "R", "data", "inst", "man", "vignettes", list.files(pkg, "\\.md$"), "_pkgdown.yml")) {
...
desc::desc_set(URL = paste0(read.dcf(file.path(stage, "DESCRIPTION"))[1, "URL"], ", https://endomer-py.github.io/enftr/en/"), file = file.path(stage, "DESCRIPTION"))
pkgdown::build_site_github_pages(pkg = stage, dest_dir = file.path(output, "en"), clean = file.exists(file.path(output, "en/pkgdown.yml")),
  install = FALSE, new_process = FALSE, examples = TRUE)
writeLines("enftr docs output", marker)
jsonlite::write_json(list(package = "enftr", version = as.character(read.dcf(file.path(pkg, "DESCRIPTION"))[1, "Version"]), languages = c("es", "en"),
